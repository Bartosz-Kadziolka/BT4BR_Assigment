...
Working_DF <- read_csv('work_files/TPMs_table_100genes.csv')
View(Working_DF)
TPM_long <- pivot_longer(Working_DF, cols = 2:7, names_to = "Sample", values_to = "TPMs")
head(TPM_long)
glimpse(TPM_long)
```
...
**Task:** Working on *TPMs_long* separate samples by "\_*" to get 1
column with condition and other with sample name. Keep original column
Sample. Create output table TPMs*\_long_1

```{r}
# YOUR CODE HERE
TPMs_long_1 <- separate(TPM_long, col = 'Sample', into = c("Condition", "Replicate Number"), sep = '_' , remove = FALSE)
head(TPMs_long_1)
```
...
```{r}
## YOUR CODE HERE
TPMs_long_2 <- select(TPMs_long_1, GeneID, 'Sample', 'Condition', 'TPMs')
```

```{r}
# YOUR CODE HERE
Out_table_e1 <- filter(TPMs_long_2, TPMs > 100)
View(Out_table_e1)
```
...
Out_table_e2 <- filter(Out_table_e1, GeneID %in% c('FBgn0000042', 'FBgn0000150', 'FBgn0000261','FBgn0000318'))
View(Out_table_e2)
```
...
```{r}
## YOUR CODE HERE
Out_table_e3 <- filter(Out_table_e2, GeneID %in% c('FBgn0000042', 'FBgn0000150', 'FBgn0000261')) |> group_by(GeneID, Condition) |> summarize(MeanTPM = mean(TPMs)) |> arrange(desc(MeanTPM))
```
...
```{r}
## YOUR CODE HERE
Table_Symbl_Gene <- read_csv('work_files/Table_Symbl_Gene.csv')
TPMs_long_2_gename <- full_join(Out_table_e3, Table_Symbl_Gene, join_by(GeneID == FLYBASE))
```

```{r}
## YOUR CODE HERE
left_join(Out_table_e3, TPMs_long_2_gename)
```

```{r}
## YOUR CODE HERE
TPMs_long_2_gename_right <- right_join(Out_table_e3, TPMs_long_2_gename)
```
...
```{r}
# YOUR CODE HERE
TPMs_long_2_gename_right <- TPMs_long_2_gename_right |> mutate(MeanTPM = replace_na(MeanTPM, 0))

TPMs_long_2_gename_right_del <- TPMs_long_2_gename_right |> drop_na()
```
...
```{r}
## YOUR CODE HERE
TEST <- TPMs_long_2 %>% group_by(GeneID, Condition) %>% summarize(SumTPM = sum(TPMs)) %>% arrange(desc(SumTPM)) %>% full_join(Table_Symbl_Gene, join_by(GeneID == FLYBASE)) %>% drop_na() %>% ungroup() %>%  slice_head(n = 10)
